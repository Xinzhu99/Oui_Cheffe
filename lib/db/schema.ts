...
export const dishes = pgTable("dishes", {
  ...
  dish_category_id: integer("dish_category_id").references(() => dish_categories.id, { onDelete: "set null" }),
...
export const menu = pgTable("menu", {
  ...
  status: text("status", { enum: ["active", "locked", "cooked"] }).notNull().default("active"),
  created_at: timestamp("created_at").defaultNow(),
})
...
export const dishCategoriesRelations = relations(dish_categories, ({ many }) => ({
  dishes: many(dishes),
}))

// Relations pour ingredient_categories
export const ingredientCategoriesRelations = relations(ingredient_categories, ({ many }) => ({
...
export const dishIngredientsRelations = relations(dish_ingredients, ({ one }) => ({
  dish: one(dishes, {
